<div class="grid">
  <div class="grid-sizer"></div>
  <!-- POST 1 - 1. Post w/ Video -->
  <div class="grid-item nightlife travel series6">
     <div class="post-thumbnail">
       <iframe width="100%" height="auto" src="https://www.youtube.com/embed/civB7P8hOdQ" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
     </div>
     <div class="post-content-wrapper">
        <div class="post-title">
           <h2 class="name">Thought pot!</h2>
        </div>
        <div class="post-category">
           Night life | Travel
        </div>
        <div class="post-content">
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cumque eos quod, voluptatum nobis unde placeat quo... <span><a href="Posts/sjefke.php">Read more</a></span>
        </div>
        <div class="post-author-date">
           <i class="far fa-user"></i> <span class="author">Yire Park</span>
           <i class="far fa-clock"></i><span class="date">11-06-2018</span>
        </div>
     </div>
  </div>
  <!-- POST 2 - Post no Image -->
  <div class="grid-item lifestyle series1">
     <div class="post-content-wrapper">
        <div class="post-title">
           <h2 class="name">When you put it that way</h2>
        </div>
        <div class="post-category">
           Lifestyle
        </div>
        <div class="post-content">
            At first glance it is just a gazebo, another tiny wooden house that dots the North West university campus landscape.... <span><a href="Posts/when-you-put-it-that-way.php">Read more</a></span>
        </div>
        <div class="post-author-date">
           <i class="far fa-user"></i> <span class="author">Faraz Saberi</span>
           <i class="far fa-clock"></i><span class="date">05-02-2019</span>
        </div>
     </div>
  </div>
  <!-- POST 3 - Post w/ soundcloud -->
  <div class="grid-item recreation series2">
     <div class="post-thumbnail">
        <iframe width="100%" height="300" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/395283300&color=%23ff5500&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true&visual=true"></iframe>
     </div>
     <div class="post-content-wrapper">
        <div class="post-title">
           <h2 class="name">3. Oh hell no, even music?!</h2>
        </div>
        <div class="post-category">
           Recreation
        </div>
        <div class="post-content">
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cumque eos quod, voluptatum nobis unde placeat quo... <span><a href="Posts/post_1.php">Read more</a></span>
        </div>
        <div class="post-author-date">
           <i class="far fa-user"></i> <span class="author">Faraz Saberi</span>
           <i class="far fa-clock"></i><span class="date">11-06-2018</span>
        </div>
     </div>
  </div>
  <!-- POST 4 - Post no Image -->
  <div class="grid-item work series3">
     <div class="post-content-wrapper">
        <div class="post-title">
           <h2 class="name">4. No image here!</h2>
        </div>
        <div class="post-category">
           Work
        </div>
        <div class="post-content">
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cumque eos quod, voluptatum nobis unde placeat quo... <span><a href="Posts/post_1.php">Read more</a></span>
        </div>
        <div class="post-author-date">
           <i class="far fa-user"></i> <span class="author">Faraz Saberi</span>
           <i class="far fa-clock"></i><span class="date">11-06-2018</span>
        </div>
     </div>
  </div>
  <!-- POST 5 - Post w/ Quote -->
  <div class="quote1 grid-item series4">
     <div class="quote-bg">
        <div class="quote-content">
           <span class="quote-mark"><i class="fas fa-quote-left fa-sm"></i></span>
           <span class="quotation"><h2 class="name">5. Take a simple idea and take it seriously</h2></span>
           <span class="quote-author">- Student</span>
        </div>
     </div>
  </div>
  <!-- POST 6 - Post w/ Gif -->
  <div class="grid-item lifestyle  series5">
     <span class="giflabel">GIF</span>
     <div class="post-thumbnail">
        <img class="gif" src="Images/1.png"><img class="active" src="Images/1.gif">
     </div>
     <div class="post-content-wrapper">
        <div class="post-title">
           <h2 class="name">6. Post with a gif!</h2>
        </div>
        <div class="post-category">
           Lifestyle
        </div>
        <div class="post-content">
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cumque eos quod, voluptatum nobis unde placeat quo... <span><a href="Posts/post_1.php">Read more</a></span>
        </div>
        <div class="post-author-date">
           <i class="far fa-user"></i> <span class="author">Rick Tijink</span>
           <i class="far fa-clock"></i><span class="date">11-06-2018</span>
        </div>
     </div>
  </div>
  <!-- POST 7 - Post w/ Video -->
  <div class="grid-item nightlife travel series6">
     <div class="post-thumbnail">
        <iframe width="100%" height="auto" src="https://www.youtube.com/embed/KnUrBMs5F6U" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
     </div>
     <div class="post-content-wrapper">
        <div class="post-title">
           <h2 class="name">7. A video post!</h2>
        </div>
        <div class="post-category">
           Night life | Travel
        </div>
        <div class="post-content">
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cumque eos quod, voluptatum nobis unde placeat quo... <span><a href="Posts/post_7.php">Read more</a></span>
        </div>
        <div class="post-author-date">
           <i class="far fa-user"></i> <span class="author">Yire Park</span>
           <i class="far fa-clock"></i><span class="date">11-06-2018</span>
        </div>
     </div>
  </div>
  <!-- POST 8 - Post w/ Image -->
  <div class="grid-item food series7">
     <div class="post-thumbnail">
        <img src="Images/1.jpg" alt="thumbnail">
     </div>
     <div class="post-content-wrapper">
        <div class="post-title">
           <h2 class="name">8. Standard post with image!</h2>
        </div>
        <div class="post-category">
           Food
        </div>
        <div class="post-content">
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cumque eos quod, voluptatum nobis unde placeat quo... <span><a href="Posts/post_1.php">Read more</a></span>
        </div>
        <div class="post-author-date">
           <i class="far fa-user"></i> <span class="author">Rick Tijink</span>
           <i class="far fa-clock"></i><span class="date">11-06-2018</span>
        </div>
     </div>
  </div>
  <!-- POST 9 - Post w/ Image -->
  <div class="grid-item food lifestyle  series8">
     <div class="post-thumbnail">
        <img src="Images/4.jpg" alt="thumbnail">
     </div>
     <div class="post-content-wrapper">
        <div class="post-title">
           <h2 class="name">9. Food Quest: Vegetarian in Xi'an</h2>
        </div>
        <div class="post-category">
           Food | Lifestyle
        </div>
        <div class="post-content">
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cumque eos quod, voluptatum nobis unde placeat quo... <span><a href="Posts/post_1.php">Read more</a></span>
        </div>
        <div class="post-author-date">
           <i class="far fa-user"></i> <span class="author">Taeyeon Kim</span>
           <i class="far fa-clock"></i><span class="date">11-06-2018</span>
        </div>
     </div>
  </div>
  <!-- POST 10 - Post w/ Gif -->
  <div class="grid-item student series1">
     <span class="giflabel">GIF</span>
     <div class="post-thumbnail">
        <img class="gif" src="Images/2.png"><img class="active" src="Images/2.gif">
     </div>
     <div class="post-content-wrapper">
        <div class="post-title">
           <h2 class="name">10. Post with a gif!</h2>
        </div>
        <div class="post-category">
           Student
        </div>
        <div class="post-content">
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cumque eos quod, voluptatum nobis unde placeat quo... <span><a href="Posts/post_1.php">Read more</a></span>
        </div>
        <div class="post-author-date">
           <i class="far fa-user"></i> <span class="author">Rick Tijink</span>
           <i class="far fa-clock"></i><span class="date">11-06-2018</span>
        </div>
     </div>
  </div>
  <!-- POST 11 - Post w/ Quote -->
  <div class="quote1 grid-item series1">
     <div class="quote-bg quoteimg">
        <div class="quote-content">
           <span class="quote-mark"><i class="fas fa-quote-left fa-sm"></i></span>
           <span class="quotation name">11. I walked around like everything was fine, but deep down inside my shoe, my sock is sliding off</span>
           <span class="quote-author">- Rick Food</span>
        </div>
     </div>
  </div>
  <!-- POST 12 - Post w/ Image -->
  <div class="grid-item nightlife student series1">
     <div class="post-thumbnail">
        <img src="Images/3.jpg" alt="thumbnail">
     </div>
     <div class="post-content-wrapper">
        <div class="post-title">
           <h2 class="name">12. Night Owls: MC for a day</h2>
        </div>
        <div class="post-category">
           Night life | Student
        </div>
        <div class="post-content">
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cumque eos quod, voluptatum nobis unde placeat quo... <span><a href="Posts/post_1.php">Read more</a></span>
        </div>
        <div class="post-author-date">
           <i class="far fa-user"></i> <span class="author">Faraz Saberi</span>
           <i class="far fa-clock"></i><span class="date">11-06-2018</span>
        </div>
     </div>
  </div>
  <!-- POST 13 - Post with Video -->
  <div class="grid-item travel series1">
      <div class="post-thumbnail">
         <iframe width="100%" height="auto" src="https://www.youtube.com/embed/5WT76lJvLSo" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
     </div>
     <div class="post-content-wrapper">
        <div class="post-title">
           <h2 class="name">13. Yo video</h2>
        </div>
        <div class="post-category">
           Travel
        </div>
        <div class="post-content">
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cumque eos quod, voluptatum nobis unde placeat quo... <span><a href="Posts/post_13.php">Read more</a></span>
        </div>
        <div class="post-author-date">
           <i class="far fa-user"></i> <span class="author">Faraz Saberi</span>
           <i class="far fa-clock"></i><span class="date">11-06-2018</span>
        </div>
     </div>
  </div>

   </div>
